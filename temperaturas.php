...
function menorTemperatura($temperaturas) {
	...
	foreach($temperaturas as $temperatura) {
		if(!isset($menorDistancia)) {
			$menorDistancia = abs($temperatura);
			$menorTemperatura = $temperatura;
		} else {
			if(abs($temperatura) < $menorDistancia || (abs($temperatura) == $menorDistancia && $temperatura > 0)) {
				$menorDistancia = abs($temperatura);
				$menorTemperatura = $temperatura;
			}
		}
...
/***** Teste EXTRA - Entrada vazia *****/
$temperaturas = array();
$resultadoEsperado = '';
$resultado = menorTemperatura($temperaturas);
verificaResultado("Extra", $resultadoEsperado, $resultado);
